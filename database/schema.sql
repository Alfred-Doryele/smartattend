...
CREATE TABLE IF NOT EXISTS users (
  ...
  temp_password TEXT,                   -- plaintext of the admin-issued temporary password, kept ONLY so an
                                         -- admin can retrieve it if they forget to relay it or the person loses
                                         -- it before ever logging in; cleared once the person resets their own
                                         -- password. NOTE: storing a plaintext password is a real security
                                         -- trade-off, accepted here for course-project account recovery
                                         -- convenience — see docs/architecture-notes.md.
...
-- A registered venue, pre-mapped once by an admin
